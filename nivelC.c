  ...
  while ((ended = waitpid(-1, &status, WNOHANG)) > 0) {
      if (jobs_list[0].pid == ended) {  //Foreground
        ...
        jobs_list[0].status = 'F';
            ...
            printf("Terminado PID %d (%s) en jobs_list[%i] con status %i \n", ended,jobs_list[pos].cmd,pos,status);
...
void ctrlc(int signum) {
  ...
  if (jobs_list[0].pid > 0) {  //Si està en foreground
    ...
    } else printf("[ctrlc()→ Señal %i no enviada debido a que el proceso en foreground es el shell]\n",SIGTERM);
  } else printf("[ctrlc()→ Señal %i no enviada debido a que no hay proceso en foreground]\n",SIGTERM);
}
...
void ctrlz(int signum){
    ...
    if(jobs_list[0].pid>0){ //Si hay proceso en foreground
        if(strcmp(jobs_list[0].cmd, mi_shell) != 0){ //Si no es el minishell
            ...
            printf("\n[ctrlz()→ Señal SIGSTOP no enviada por %d (%s) debido a que el proceso en foreground es el shell]\n",getpid(),mi_shell);
           
        }
    }else{
        sprintf(mensaje, "\n[ctrlz()→ Señal SIGSTOP no enviada por %d debido a que no hay proceso en foreground]\n",getpid());
            write(2, mensaje, strlen(mensaje)); 
    }
}
